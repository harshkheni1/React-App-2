...
import { Button, notification, Form, Select, Input, Table, Modal, Spin } from 'antd'
...
const Prescription = (patientData) => {
  ...
  const getPharmacyList = async () => {
    try {
      const pharmacyListReponse = await GET(`pharmacy/getpharmacylist?pharmacy_query=`)
      const parsedResponse = JSON.parse(pharmacyListReponse?.data?.body)
      if (parsedResponse?.pharmacy_list.length > 0) {
        setPharmacyList(parsedResponse?.pharmacy_list)
      } else {
        setPharmacyList([])
      }
    } catch (error) {
  ...
  const getDrugList = async (drugName) => {
    try {
      const drugListReponse = await GET(`drugs/getdrugslist?drug_query=${drugName}`)
      const parsedResponse = JSON.parse(drugListReponse?.data?.body)

      if (parsedResponse?.drug_list.length > 0) {
        setDrugsList(parsedResponse?.drug_list)
      } else {
        setDrugsList([])
      }
    } catch (error) {
